...
| activation_date_key | Date of first eligible post-win Order, or a governed technical member. | ANALYTICAL_KEY | NO | Derived Order Item event | Resolve first qualifying purchase date; use `NOT_OBSERVED` for a valid seller without an observed event and `UNKNOWN` for invalid temporal sequence. | Must be calendar member iff activation timestamp is present. |
| activation_timestamp | First delivered Order purchase strictly after won timestamp. | TIMESTAMP | YES | Derived from Order Items + Orders | Deduplicate at seller/order grain, then select minimum purchase timestamp with order-id tie-break. | May fall after 90 days; NULL when no valid temporal derivation is available. |
...
| observation_cutoff_timestamp | Inclusive upper bound of e-commerce evidence used by the evaluation. | TIMESTAMP | NO | Governed run metadata | Maximum observed Order purchase timestamp for the compatible snapshot. | A later Closed Deal is retained as immature rather than dropped. |
...
| temporal_quality_status | Governed contact-to-close temporal quality inherited from acquisition. | STRING | NO | Closed Deal | Preserved from `fct_closed_deal`. | `INVALID_SEQUENCE` rows remain present but cannot produce temporal lifecycle metrics. |
| is_mature_90d | Whether a valid temporal sequence has 90 complete observation days. | BOOLEAN | NO | Derived | TRUE when status is `VALID` and cutoff is at least won timestamp plus 90 days. | Controls activation-rate denominator eligibility; invalid sequence is FALSE. |
| is_activated_90d | Tri-state activation result. | BOOLEAN | YES | Derived | TRUE only for a mature valid seller whose first eligible event is within 90 days; FALSE only for mature valid sellers without such an event; NULL otherwise. | Never FALSE for immature or invalid-temporal rows. |
...
| time_to_first_order_days | Elapsed fractional days to the first eligible post-win Order. | DECIMAL | YES | Derived | Difference between won timestamp and activation timestamp. | Greater than 0 when present; non-additive; NULL for invalid sequence or no event. |
| first_eligible_order_id | First delivered post-win Order after seller/order deduplication. | STRING | YES | Derived | Deterministic minimum qualifying purchase timestamp, then order_id tie-break. | Required exactly when activation timestamp is present. |
